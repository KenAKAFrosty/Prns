...
//! RNS 1.3.5 `Destination._persist_ratchets` writes each destination's list to open disk, signed by the destination identity; ours seals the same record and stores it as a vault blob beside the identity secret itself.
