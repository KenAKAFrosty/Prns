...
use crate::interfaces::{ConfiguredInterfacePolicy, InterfaceMode};

fn decoded(encoded: &[u8]) -> Vec<u8> {
    let mut decoder = RnsSerialDecoder::<{ core::WDCL_MAX_CHUNK }>::new();
...
fn defaults_match_the_reference_weave_medium() {
    ...
    assert_eq!(policy.bitrate, core::WEAVE_BITRATE_ESTIMATE);
    assert_eq!(policy.mode, InterfaceMode::Full);
    assert_eq!(policy.mtu.resolve(policy.bitrate), Some(core::WEAVE_HW_MTU));
}
...
fn discovery_broadcast_contains_the_host_switch_id() {
    ...
    let written = wdcl::encode_discovery(&identity, &mut output).unwrap();
    let frame = decoded(&output[..written]);
    assert_eq!(&frame[..SwitchId::LEN], BROADCAST_SWITCH.as_bytes());
    assert_eq!(frame[SwitchId::LEN], wdcl::TYPE_DISCOVER);
...
#[test]
fn handshake_matches_the_rns_1_3_8_python_oracle() {
    let identity = WeaveHostIdentity::from_signing_secret([0x11; 32]);
    let remote = SwitchId::new([0x34, 0x55, 0xa4, 0xf0]);
    let mut output = [0u8; 256];
    let written = wdcl::encode_handshake(&identity, remote, &mut output).unwrap();
    assert_eq!(
        &output[..written],
        from_hex(
...
#[test]
fn discovery_response_verification_matches_the_rns_1_3_8_python_oracle() {
    let frame = from_hex(
    ...
    assert_eq!(
        wdcl::decode_device_frame(&frame, SwitchId::new([0xc9, 0x77, 0x87, 0x37])),
    ...
    let written = wdcl::encode_discovery_response(
        &device,
...
fn discovery_rejects_a_signature_for_a_different_host_switch() {
    ...
    assert_eq!(
        wdcl::decode_device_frame(&frame, SwitchId::new([0xc9, 0x77, 0x87, 0x37])),
        Err(wdcl::DecodeError::InvalidDiscoverySignature)
    );
}
...
fn endpoint_command_carries_the_typed_destination_and_packet() {
    ...
    let written =
        wdcl::encode_endpoint_packet(switch, endpoint, b"packet", &mut raw, &mut output).unwrap();
    let frame = decoded(&output[..written]);
    assert_eq!(&frame[..4], switch.as_bytes());
    assert_eq!(frame[4], wdcl::TYPE_COMMAND);
    assert_eq!(&frame[5..7], &wdcl::COMMAND_ENDPOINT_PACKET.to_be_bytes());
...
fn endpoint_command_retains_ifac_headroom_beyond_the_fixed_mtu() {
    let payload = vec![0x42; core::WEAVE_MAX_WIRE_PACKET];
    ...
    let written = wdcl::encode_endpoint_packet(
...
fn endpoint_packet_exposes_source_and_payload_without_copying() {
    ...
    let mut frame = Vec::from([0x11, 0x12, 0x13, 0x14, wdcl::TYPE_ENDPOINT_PACKET]);
    frame.extend_from_slice(b"packet");
    frame.extend_from_slice(source.as_bytes());
    assert_eq!(
        wdcl::decode_device_frame(&frame, SwitchId::new([0x11, 0x12, 0x13, 0x14])),
...
fn endpoint_packet_for_another_switch_is_not_delivered() {
    ...
    assert_eq!(
        wdcl::decode_device_frame(&frame, SwitchId::new([0x21, 0x22, 0x23, 0x24])),
