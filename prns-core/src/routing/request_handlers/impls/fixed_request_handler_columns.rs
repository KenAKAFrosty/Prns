...
pub struct FixedRequestHandlerColumns<const MAX_REQUEST_HANDLERS: usize> {
...
impl<const MAX_REQUEST_HANDLERS: usize> RequestHandlerColumns
    for FixedRequestHandlerColumns<MAX_REQUEST_HANDLERS>
{
    ...
    fn allow_at(&mut self, slot: usize, identity: IdentityHash) -> Result<(), ColumnsFull> {
        ...
        list.push(identity).map_err(|_| ColumnsFull)
    }
