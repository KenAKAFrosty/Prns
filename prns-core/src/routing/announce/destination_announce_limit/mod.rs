...
pub trait DestinationAnnounceLimitColumns {
...
pub struct DestinationAnnounceLimits<C: DestinationAnnounceLimitColumns> {
    columns: C,
}

impl<C: DestinationAnnounceLimitColumns> DestinationAnnounceLimits<C> {
...
mod tests {
    ...
    fn a_first_sighting_is_always_allowed() {
        let mut rates: DestinationAnnounceLimits<FixedDestinationAnnounceLimitColumns<4>> =
    ...
    fn eviction_drops_the_least_recently_active() {
        let mut rates: DestinationAnnounceLimits<FixedDestinationAnnounceLimitColumns<2>> =
            DestinationAnnounceLimits::default();
    ...
    fn capacity_zero_stores_nothing_and_never_blocks() {
        let mut rates: DestinationAnnounceLimits<FixedDestinationAnnounceLimitColumns<0>> =
            DestinationAnnounceLimits::default();
        let l = limit(10_000, 0, 60_000);
        assert_eq!(
            rates.observe(dest(1), InstantMillis(0), l),
    ...
    fn heap_columns_track_past_any_fixed_ceiling() {
        let mut rates: DestinationAnnounceLimits<HeapDestinationAnnounceLimitColumns> =
            DestinationAnnounceLimits::default();
        let l = limit(10_000, 0, 60_000);
        for n in 0..64u8 {
            assert_eq!(
    ...
    fn insert_reports_whether_the_entry_could_be_recorded() {
        let mut holds: FixedDestinationAnnounceLimitColumns<2> =
            FixedDestinationAnnounceLimitColumns::default();
        assert_eq!(
            holds.insert(dest(1), DestinationAnnounceLimit::default()),
            DestinationAnnounceLimitAdmission::Recorded
        );
        let mut holds_nothing: FixedDestinationAnnounceLimitColumns<0> =
            FixedDestinationAnnounceLimitColumns::default();
        assert_eq!(
