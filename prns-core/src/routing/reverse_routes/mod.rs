...
pub struct ReverseRoutes<C: ReverseRouteColumns> {
    columns: C,
}

impl<C: ReverseRouteColumns> ReverseRoutes<C> {
    ...
    pub fn take(
        ...
        let expired = self.columns.expires_ats()[index] <= now;
        let route = ReverseRoute {
            received_interface: self.columns.received_interfaces()[index],
            outbound_interface: self.columns.outbound_interfaces()[index],
        };
...
mod tests {
    ...
    fn a_full_table_evicts_the_soonest_expiring_row_for_the_new_packet() {
        let mut table: ReverseRoutes<FixedReverseRouteColumns<2>> = ReverseRoutes::default();
    ...
    fn heap_columns_grow_past_any_fixed_ceiling() {
        let mut table: ReverseRoutes<HeapReverseRouteColumns> = ReverseRoutes::default();
        for n in 0..64u8 {
