...
/// The absolute ceiling on a negotiated link MTU: RNS 1.4.0's top `optimise_mtu` tier (524288, what a ≥1 Gbps wire wants). A safety bound, not a per-interface size: a link negotiates its own interface's `hardware_mtu` (see `link_mtu_ceiling`), and neither host nor embedded buffers pay this ceiling (they size per interface, or to their own hardware).
