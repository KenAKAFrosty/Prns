//! RNS 1.4.0 Resources: the sender seals the whole stream once under the session key and slices the ciphertext into parts; the receiver pulls parts by 4-byte map hashes inside a sliding window. The link is the authentication; no signature rides the transfer.
...
/// RNS 1.4.0 `Resource.MAPHASH_LEN`: a part is named by the first four bytes of `full_hash(part ‖ salt nonce)`.
pub const MAP_HASH_LEN: usize = 4;

/// RNS 1.4.0 `Resource.RANDOM_HASH_SIZE`.
///
...
/// A resource names itself by a full SHA-256. RNS 1.4.0 `Identity.full_hash(data + random_hash)`
pub const RESOURCE_HASH_LEN: usize = SHA256_OUTPUT_LEN;

/// RNS 1.4.0 `Resource.WINDOW_MAX` (= `WINDOW_MAX_FAST`). The widest part window either end will ever run. The collision guard is sized from it.
pub const WINDOW_MAX: usize = 75;

/// RNS 1.4.0 `Resource.WINDOW`
pub const WINDOW_START: usize = 4;

/// RNS 1.4.0 `Resource.WINDOW_MIN`.
pub const WINDOW_MIN: usize = 2;

/// RNS 1.4.0 `Resource.WINDOW_MAX_SLOW`. The ceiling a window grows toward until the link proves fast enough to lift it to [`WINDOW_MAX`].
pub const WINDOW_MAX_SLOW: usize = 10;

/// RNS 1.4.0 `Resource.WINDOW_FLEXIBILITY`. How far the window may run ahead of its floor before the floor follows it up.
pub const WINDOW_FLEXIBILITY: usize = 4;

/// RNS 1.4.0 `Resource.MAX_RETRIES`
pub const PART_REQUEST_MAX_RETRIES: u8 = 16;

/// RNS 1.4.0 `Resource.MAX_ADV_RETRIES`
pub const MAX_ADVERTISEMENT_RETRIES: u8 = 4;

/// RNS 1.4.0 `Resource.PART_TIMEOUT_FACTOR`. The rtt multiple a receiver waits on outstanding parts before retrying.
pub const PART_TIMEOUT_FACTOR: u64 = 4;

/// RNS 1.4.0 `Resource.PART_TIMEOUT_FACTOR_AFTER_RTT`.
pub const PART_TIMEOUT_FACTOR_AFTER_RTT: u64 = 2;

/// RNS 1.4.0 `Resource.RATE_FAST` (50 kbps as bytes/s). The measured rate past which a round counts toward lifting the window ceiling to  [`WINDOW_MAX`].
pub const RATE_FAST_BYTES_PER_SECOND: u64 = 50 * 1000 / 8;

/// RNS 1.4.0 `Resource.RATE_VERY_SLOW` (2 kbps as bytes/s): the measured rate below which a round counts toward dropping the ceiling to [`WINDOW_MAX_VERY_SLOW`].
pub const RATE_VERY_SLOW_BYTES_PER_SECOND: u64 = 2 * 1000 / 8;

/// RNS 1.4.0 `Resource.WINDOW_MAX_VERY_SLOW`.
pub const WINDOW_MAX_VERY_SLOW: usize = 4;

/// RNS 1.4.0 `Resource.FAST_RATE_THRESHOLD` (`WINDOW_MAX_SLOW - WINDOW - 2` = 4). How many fast rounds earn the lift.
pub const FAST_RATE_THRESHOLD: u8 = (WINDOW_MAX_SLOW - WINDOW_START - 2) as u8;

/// RNS 1.4.0 `Resource.VERY_SLOW_RATE_THRESHOLD`. How many very-slow rounds (with no fast round ever seen) drop the ceiling.
...
/// RNS 1.4.0 `Resource.PROOF_TIMEOUT_FACTOR`. The smaller rtt multiple a sender waits on the proof.
pub const PROOF_TIMEOUT_FACTOR: u64 = 3;

/// RNS 1.4.0 `Resource.PROCESSING_GRACE`
pub const PROCESSING_GRACE_MS: u64 = 1_000;

/// RNS 1.4.0 `Resource.RETRY_GRACE_TIME`
pub const RETRY_GRACE_MS: u64 = 250;

/// RNS 1.4.0 `Resource.PER_RETRY_DELAY`
pub const PER_RETRY_DELAY_MS: u64 = 500;

/// RNS 1.4.0 `Resource.SENDER_GRACE_TIME`
...
/// RNS 1.4.0 `ResourceAdvertisement.OVERHEAD`: the byte budget the reference reserves for everything in a packed advertisement except the map hashes.
pub const ADVERTISEMENT_OVERHEAD: usize = 134;

/// RNS 1.4.0 `ResourceAdvertisement.HASHMAP_MAX_LEN` (74): how many map hashes ride one advertisement or one hashmap update.
///
...
/// RNS 1.4.0 `ResourceAdvertisement.COLLISION_GUARD_SIZE` (224): The sliding span of parts within which two map hashes must not collide.
/// The sender re-rolls its salt nonce until they don't.
pub const COLLISION_GUARD_SIZE: usize = 2 * WINDOW_MAX + HASHMAP_MAX_LEN;

/// RNS 1.4.0 `Resource.MAX_EFFICIENT_SIZE` (1 MiB − 1). The most one segment carries.
///
...
/// RNS 1.4.0 `Resource.METADATA_MAX_SIZE` (16 MiB − 1)
pub const METADATA_MAX_SIZE: usize = 16 * 1024 * 1024 - 1;

/// The length prefix ahead of the packed metadata in the stream: `struct.pack(">I", metadata_size)[1:]` (RNS 1.4.0 `Resource.__init__`).
pub const METADATA_PREFIX_LEN: usize = 3;

/// RNS 1.4.0 `Resource.sdu`
...
/// RNS 1.4.0 `Resource.get_map_hash`; the four-byte name a part is requested by `full_hash(part ‖ salt nonce)` truncated.
...
/// RNS 1.4.0 `Link.resource_strategy`, engine-gated: the reference's unbounded `ACCEPT_ALL` becomes an accept with enforced bounds, refused at the advertisement gate before a single part moves.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum ResourceStrategy {
    /// Every link is born refusing resources; RNS 1.4.0 `Link.__init__` sets `ACCEPT_NONE`.
    #[default]
    AcceptNone,
    Accept {
...
/// RNS 1.4.0 hands the `ACCEPT_APP` callback the parsed advertisement; this is that view: one unsolicited segment's facts, judged before a single part moves.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl ResourceCompression {
    ...
    /// bz2 is the only compression RNS 1.4.0 can mean by the `c` flag.
    #[must_use]
...
/// RNS 1.4.0 resource metadata on the send side: packed msgpack bytes the engine never unpacks,  carried at the head of the stream as `3-byte-BE-length ‖ packed` and covered by the resource hash, the advertised `d`, and (when one wins) the compressed stream.
///
...
/// RNS 1.4.0 advertises `(segment_index, total_segments)` plus the whole transfer's uncompressed length (the `d` field) on every segment.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// RNS 1.4.0 `expected_proof = Identity.full_hash(data + hash)`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
