...
impl<
        ...
        const MAX_PARTS: usize,
        A: Allocator + Default,
    > Default for FixedHeapResourceColumns<State, SLOTS, TRANSFER_BYTES, MAX_PARTS, A>
{
    fn default() -> Self {
        const {
            assert!(
...
mod tests {
    ...
    type Columns = FixedHeapResourceColumns<u8, 2, 1024, 3>;
    ...
    fn push_then_read_back_and_seal_into_the_boxed_transfer() {
        ...
        let i = columns.push(link(1), hash(0xA1), 7).unwrap();
        columns.set_hash(i, hash(0xB2));
        let buffers = columns.buffers_mut(i);
        buffers.transfer[..4].copy_from_slice(&[1, 2, 3, 4]);
        buffers.part_flags[0] = true;

        assert_eq!(columns.len(), 1);
        assert_eq!(columns.hashes(), &[hash(0xB2)]);
        assert_eq!(&columns.transfer(i)[..4], &[1, 2, 3, 4]);
        assert!(columns.part_flags(i)[0]);
        assert_eq!(columns.states(), &[7]);
    }

    #[test]
    fn a_full_table_refuses_the_next_push() {
        let mut columns = Columns::default();
        columns.push(link(1), hash(1), 1).unwrap();
        columns.push(link(2), hash(2), 2).unwrap();
        assert_eq!(
            columns.push(link(3), hash(3), 3),
    ...
    fn swap_remove_moves_the_last_slot_and_clears_its_state() {
        let mut columns = Columns::default();
        let a = columns.push(link(1), hash(1), 11).unwrap();
        columns.push(link(2), hash(2), 22).unwrap();
        columns.buffers_mut(a).transfer[0] = 0xEE;

        columns.swap_remove(a);

        assert_eq!(columns.len(), 1);
        assert_eq!(columns.states(), &[22]);
        assert_eq!(columns.link_ids(), &[link(2)]);
    }
}
