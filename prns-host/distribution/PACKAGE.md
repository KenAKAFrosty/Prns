...
Every hosted SDK delegates protocol behavior to the same engine. Native FFI packages use the versioned C ABI; Node and Bun talk directly to the native Rust host; browsers run the cooperative WebAssembly engine on a DedicatedWorker by default and transfer packed event projections to the page. Language packages own types, deterministic lifetime, cancellation, and ecosystem-native streams; they do not reimplement routing or wire semantics.
