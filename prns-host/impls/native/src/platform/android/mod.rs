//! Android platform Bluetooth bridge ownership for any native host composition.
//!
...
/// Reserve the next platform callback generation before starting Kotlin BLE pumps.
pub fn prepare_bluetooth() -> bool {
    owner().prepare()
}

/// Release unused preparation. A live attached session must finish shutdown first.
pub fn release_bluetooth() -> bool {
    owner().release()
}
...
pub struct BluetoothSession {
    lease: Arc<BluetoothLease>,
}

#[cfg(any(test, target_os = "android"))]
struct BluetoothLease {
    owner: Arc<BluetoothOwner>,
}

/// A prepared Android transport that retains its bridge reservation until the
/// actual supervisor future is dropped, even when detach only enqueues stop.
#[cfg(any(test, target_os = "android"))]
pub struct AndroidBluetoothInterface {
    inner: personal_rns::bluetooth_auto::BluetoothAuto<
        AndroidBleBackend,
...
#[cfg(any(test, target_os = "android"))]
impl AndroidBluetoothInterface {
    pub fn status(&self) -> personal_rns::bluetooth_auto::BluetoothAutoStatus {
        self.inner.status()
    }
}

#[cfg(any(test, target_os = "android"))]
impl personal_rns::interfaces::ReportsStatus for AndroidBluetoothInterface {
    fn status_view(&self) -> Option<personal_rns::interfaces::StatusView> {
        self.inner.status_view()
    }
}

#[cfg(any(test, target_os = "android"))]
impl personal_rns::runtime::InterfaceSupervisor for AndroidBluetoothInterface {
    const KIND: personal_rns::interfaces::InterfaceKind =
        personal_rns::interfaces::InterfaceKind::BluetoothAuto;

    fn channel_tag(&self) -> &[u8] {
        self.inner.channel_tag()
    }

    fn policy(&self) -> personal_rns::interfaces::EffectiveInterfacePolicy {
        self.inner.policy()
    }

    async fn run(self, fleet: personal_rns::runtime::Fleet) {
        let Self { inner, lease } = self;
        inner.run(fleet).await;
        drop(lease);
    }
}
...
impl BluetoothSession {
    pub fn bridge(&self) -> AndroidBleBridge {
        self.lease.owner.bridge.clone()
    }

    pub fn interface(
        &self,
        identity: personal_rns::interfaces::bluetooth_auto::BleIdentity,
    ) -> AndroidBluetoothInterface {
        ...
        let inner = personal_rns::bluetooth_auto::BluetoothAuto::new(
...
#[cfg(any(test, target_os = "android"))]
pub fn take_prepared_bluetooth() -> Option<BluetoothSession> {
    owner().take_prepared()
}
...
mod tests {
    ...
    #[test]
    fn interface_holds_generation_after_session_owner_releases_it() {
        use personal_rns::interfaces::bluetooth_auto::BleIdentity;

        let owner = Arc::new(BluetoothOwner::new());
        assert!(owner.prepare());
        let session = owner.take_prepared().expect("prepared session");
        let interface = session.interface(BleIdentity::new([0x53; 16]));
        drop(session);
        assert!(!owner.prepare());
        assert!(!owner.release());
        drop(interface);
