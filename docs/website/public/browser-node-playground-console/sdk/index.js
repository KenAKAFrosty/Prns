        ...
        switch (message.type) {
            ...
            case "data":
                if (this.#confirmed && message.bytes.length > 0) {
                    const ingested = this.#host.ingest(this.interfaceId, packetFrame(message.bytes));
...
export class Prns {
    ...
    #entropy;
    #now;
    constructor(wasm, runtime, entropy, now, bleIdentityAvailability) {
        this.#runtime = runtime;
        this.#entropy = entropy;
        this.#now = now;
        this.interfaces = new PrnsInterfaces(new RuntimeHost(wasm, runtime, entropy, now, bleIdentityAvailability));
    }
    static async create(options) {
        let identityLength;
        try {
            identityLength = positiveInteger(options.wasm.identitySecretKeyLength(), "identity secret key length");
        }
        catch (error) {
        ...
        const bleIdentity = bleIdentityAvailability.tag === "Available"
            ...
            : undefined;
        try {
            return Tag("Ready", new Prns(options.wasm, new options.wasm.PrnsRuntime(identity, bleIdentity), options.entropy ?? webCryptoEntropy, options.now ?? nowMillis, bleIdentityAvailability));
        }
        catch (error) {
            return runtimeRejected("initialize", error);
        }
    ...
    announce(destination) {
        ...
        try {
            return Tag("Queued", commandId(this.#runtime.announce({
                destination,
                nowMs: this.#now(),
...
function describeInterfaceSessionFailure(failure) {
    switch (failure.tag) {
        ...
        case "InsufficientEntropy":
            return (`entropy source returned ${failure.data.actual} bytes; ` +
