...
# 页脚
footer-tagline = 由 KenAKAFrosty 与 Personal/Prns 团队带来。
...
landing-subtitle = 为每个 Reticulum 节点所需的性能、稳定性与能效而构建，从五美元的微控制器到云服务器集群。同一个引擎、同一套 API，在嵌入式、桌面、移动、游戏和 Web 上完全一致。
...
interfaces-host-body = TCP client/server、UDP、WebSocket 和 Backbone 让远端 peers 也能通过 private WAN、VPN、public Internet relay 和浏览器集成参与 mesh。
...
standards-correctness-body = 每一次改动都会与参考实现核对，然后经过单元测试、属性测试、模糊测试和 mutation 测试，在关键之处还会加入 Kani 证明。
...
start-section-lead = 按 Prns 进入你工作的方式选择路径：要烧录的硬件、要运行的基础设施，或要构建的软件。
...
platforms-lead = 一个引擎，多个归宿。这份速览把运行时平台支持与具体 Hopspot 开发板支持分开呈现。
