...
standards-safety-body = Cratene våre inneholder null unsafe, håndhevet av kompilatoren. Unsafe i avhengigheter sjekkes for UB under Miri og auditeres med cargo-geiger.
...
start-lxmf-body = LXMF på toppen av Reticulum — identiteter, adresser, levering. Laget Sideband og Nomadnet ligger på.
...
crate-rnsd-blurb = En drop-in for rnsd som kjører overalt hvor Linux kjører. Samme wire som RNS-referansen; bruk den ved siden av eller i stedet for nodene du allerede har.
crate-lxmf-role = Meldinger
crate-lxmf-blurb = LXMF på toppen av Reticulum — laget Sideband og Nomadnet ligger på. Identiteter, adresser, meldingslevering.
