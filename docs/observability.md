...
`prns.announces.pacer_deferred_depth` and `prns.announces.pacer_oldest_deferred_age_ms` show current retained work node-wide. Their interface views are `prns.interface.announces.queue_depth{queue="pacer_deferred"}` and `prns.interface.announces.oldest_deferred_age_ms`. Physical outbound capacity is exported as `prns.egress.lane.capacity` and `prns.egress.lane.occupancy`, labeled with both the physical lane and its logical interface. The same labels appear on `prns.egress.lane.pending`; node-wide `prns.egress.pending_frames`, `prns.egress.maximum_pending_frames`, and `prns.egress.pending_stalled_seconds` expose deferred work and its lack of flush progress. A nonempty pending queue that makes no progress for 30 seconds also emits the structured `egress_pending_stalled` warning once, resetting after progress or recovery. A `lane_full` outcome is a terminal failure only for a pacerless announce or a full bounded pending queue; retained pacer work is represented exclusively by the backpressure lifecycle.
