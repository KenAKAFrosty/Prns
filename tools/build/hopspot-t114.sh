...
output="$root/target/hopspot-t114"
cargo_target="$output/cargo"
elf="$cargo_target/thumbv7em-none-eabihf/release/heltec-t114"
binary="$output/heltec-t114.bin"
uf2="$output/heltec-t114.uf2"
...
if [[ -z "$application_base" ]]; then
    echo "the T114 ELF does not contain .vector_table" >&2
    exit 1
fi
