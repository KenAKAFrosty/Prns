    ...
    DLY: DelayNs,
{
    ...
    pub async fn transmit(&mut self, payload: &[u8]) -> Result<(), Error> {
        ...
        // DIO1 can still be physically high for a short time after clearing a receive-side IRQ.
        // Starting TX before it releases lets the subsequent wait consume that stale level and
        // report success before the radio has actually transmitted anything.
...
mod tests {
    ...
    fn a_stale_dio1_level_must_release_before_set_tx() {
        ...
        assert!(
            ...
            "SetTx must not start while DIO1 is still reporting a stale IRQ"
        );
    }
