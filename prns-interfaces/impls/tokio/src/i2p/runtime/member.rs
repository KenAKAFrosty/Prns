...
{
    const HW_MTU: usize = core::I2P_HW_MTU;
        ...
        loop {
            if session.is_none() {
                ...
                let id = match generate_session_id() {
                    Ok(id) => id,
                    Err(error) => {
                        self.status.set_issue(I2pRuntimeIssue::EntropyUnavailable);
...
fn issue_for(error: &impl SamTransportError) -> I2pRuntimeIssue {
    match error.failure_class() {
        SamFailureClass::SamUnavailable => I2pRuntimeIssue::SamUnavailable,
        SamFailureClass::PeerUnreachable => I2pRuntimeIssue::PeerUnreachable,
    }
}
