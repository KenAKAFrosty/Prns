...
use prns_core::interfaces::weave::core;
use prns_core::interfaces::weave::wdcl::{
        ...
        loop {
            ...
            let stream = match (self.open)().await {
                ...
                Err(error) => {
                    ...
                    self.status.set_issue(WeaveRuntimeIssue::SerialUnavailable);
            ...
            let issue = match result {
                Err(error) if error.kind() == io::ErrorKind::TimedOut => {
                    ...
                    WeaveRuntimeIssue::HandshakeTimedOut
                }
                Err(error) => {
                    crate::diagnostic_log::warn!(
                    ...
                    WeaveRuntimeIssue::ConnectionLost
                }
...
where
    Stream: AsyncRead + AsyncWrite + Unpin,
{
    let mut framed = vec![0u8; core::FRAMED_LEN];
    let discovery_len = wdcl::encode_discovery(identity, &mut framed).map_err(encoding_error)?;
    ...
    let mut decoder = Box::new(RnsSerialDecoder::<{ core::WDCL_MAX_CHUNK }>::new());
    let mut read = vec![0u8; core::READ_BUF_LEN];
    let mut raw = vec![0u8; SwitchId::LEN + 1 + 2 + EndpointId::LEN + core::WEAVE_MAX_WIRE_PACKET];
        ...
        tokio::select! {
            ...
            read_result = stream.read(&mut read) => {
                ...
                while offset < read_len {
                    ...
                    let event = match wdcl::decode_device_frame(frame, identity.switch_id()) {
                        Ok(event) => event,
                    ...
                    match event {
                        DeviceEvent::Discovered { switch_id, .. } => {
                            ...
                            let written = wdcl::encode_handshake(identity, switch_id, &mut framed)
                                .map_err(encoding_error)?;
                            stream.write_all(&framed[..written]).await?;
                        }
    ...
    let expired = peers
        ...
        .filter_map(|(endpoint, peer)| {
            (now.duration_since(peer.last_heard).as_millis()
                > u128::from(core::PEERING_TIMEOUT_MILLIS))
            .then_some(*endpoint)
        })
        .collect::<Vec<_>>();
...
fn encoding_error(error: wdcl::EncodeError) -> io::Error {
    io::Error::new(
