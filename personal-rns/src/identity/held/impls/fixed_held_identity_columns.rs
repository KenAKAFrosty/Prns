...
pub struct FixedHeldIdentityColumns<const MAX_HELD_IDENTITIES: usize> {
...
impl<const MAX_HELD_IDENTITIES: usize> Default for FixedHeldIdentityColumns<MAX_HELD_IDENTITIES> {
    fn default() -> Self {
        Self {
...
impl<const MAX_HELD_IDENTITIES: usize> HeldIdentityColumns
    for FixedHeldIdentityColumns<MAX_HELD_IDENTITIES>
{
...
mod tests {
    ...
    fn exposes_only_pushed_rows_and_reports_a_full_store() {
        let mut columns = FixedHeldIdentityColumns::<2>::default();
        assert_eq!(columns.capacity(), 2);
        assert!(columns.is_empty());
        assert!(columns.hashes().is_empty());
        assert!(columns.encryption_secret_at(0).is_none());
        ...
        assert_eq!(
            columns.push(hash_a, enc_a, sig_a, enc_pub_a, sig_pub_a),
            Ok(0)
        );
        assert_eq!(
            columns.push(hash_b, enc_b, sig_b, enc_pub_b, sig_pub_b),
            Ok(1)
        );
        assert_eq!(
            columns.push(hash_c, enc_c, sig_c, enc_pub_c, sig_pub_c),
            Err(HoldIdentityError::StoreFull),
        );

        assert_eq!(columns.len(), 2);
        assert_eq!(columns.hashes(), &[hash_a, hash_b]);
        assert_eq!(columns.encryption_publics(), &[enc_pub_a, enc_pub_b]);
        assert_eq!(columns.signing_publics(), &[sig_pub_a, sig_pub_b]);
        assert!(columns.encryption_secret_at(1).is_some());
        assert!(columns.signing_secret_at(1).is_some());
        assert!(columns.encryption_secret_at(2).is_none());
    }
}
