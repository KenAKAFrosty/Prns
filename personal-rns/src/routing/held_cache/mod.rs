...
//!
//! Entries are keyed by destination, returned lowest-hops-first, and fully
...
/// Per-entry inline budget for held announce `app_data`.
pub const HELD_APP_DATA_LIMIT: usize = MTU - HEADER_LEN - ANNOUNCE_FIXED_FIELDS_LEN;

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ParkOutcome {
    Parked,
    /// Same destination was already held and got replaced.
    Overwrote,
    /// Capacity was reached for a new destination.
    CacheFull,
    /// `announce.app_data.len() > HELD_APP_DATA_LIMIT`.
    AppDataTooLarge,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum HoldReason {
    /// The routing-table `app_data` arena was full at upsert time.
    #[default]
    RoutingArenaPressure,
}

/// Owned snapshot of a held entry.
...
/// Fixed-capacity held-announce store.
#[derive(Debug, Clone, PartialEq, Eq)]
