...
/// the predicate reads as intent rather than a bare flag. Only an
/// `Unresponsive` incumbent can be failed over from at equal evidence.
...
/// The fields of an existing route the acceptance predicate consults,
/// gathered from the table's columns on a lookup hit. The history is a
/// borrowed two-slice view ([`AnnounceIdHistoryView`]) over the tiered
/// store, so the predicate reads it in place.
