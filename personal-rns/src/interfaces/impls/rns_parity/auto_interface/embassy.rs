...
use crate::interfaces::{
    ...
    InterfaceStats, InterfaceWorker, InterfaceWorkerContext, LinkState, MacAddress, MediumKind,
    QueueFull, TrackedPeerMulticastInterface,
};
...
/// path reads this to build its `SelfDrivenInterface` (the legacy
/// [`EmbassyAutoInterface::new`] builds the same descriptor inline).
...
/// The AutoInterface worker over the contract seam — the going-forward twin of
/// [`run`]. Same three UDP sockets + discovery brain + beacon; re-plumbed so inbound
/// data packets ride [`InboundSink::submit`] and the runtime's outbound is pulled with
/// `ready` + `try_next_into`, then fanned to every peer. No `link_up`/`peers` atomics —
/// liveness + peer count ride a control report under the new contract (deferred); the
/// brain still logs its live peer count each beacon.
///
/// Generic over the seam `DEPTH`. `stack` must already be up (host brought up WiFi +
/// IP). Unlike [`run`] it takes no `id` — the seam tags inbound provenance.
