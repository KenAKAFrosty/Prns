...
/// What one pass of the contract-seam RX/TX select resolved to — the seam twin of
/// [`Step`]. `OutboundReady` carries no packet: with the seam the transmit branch
/// pulls it after the `rx` borrow on the radio is released.
enum ServeStep {
    Received(usize),
    ReceiveFailed,
    OutboundReady,
...
/// Drive the LoRa link forever over the contract seam — the going-forward twin of
/// [`run`]. Same half-duplex loop: arm continuous RX and wait for either a received
/// frame (reassemble RNode's split → `submit` the whole packet) or an outbound packet
/// (frame it, splitting RNode-style if needed → transmit, then return to RX).
///
