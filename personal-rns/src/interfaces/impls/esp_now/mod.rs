...
//! larger v2 frame (1470 B vs. the 500 B MTU) lets the worker coalesce several
//! packets into one transmission instead of fragmenting.
//!
//! [`core`] is the platform-agnostic part: the routing descriptor and the
//! coalescing frame codec. The embassy worker drives esp-radio's ESP-NOW
//! sender/receiver behind an `EspNowLink` seam for hosts that have the radio.
