/// Connection lifecycle state of an interface, observable by the
/// engine. Used for routing decisions (the engine prefers `Connected`
/// interfaces over `Degraded` ones, won't route via `Failed` or
/// `Disconnected`). Used for diagnostics, and - critically - for
/// hot-reload-without-restart support: the host can move an interface
/// through `Disconnected` and remove it without taking down the engine,
/// and add new interfaces in `Initializing` at any time.
///
