...
use crate::engine::self_ratchets::{LastRotated, SelfRatchetColumns, TrackRatchetsError};
use crate::engine::InstantMillis;
use crate::wire::DestinationHash;
use heapless::Vec as HeaplessVec;

pub struct FixedSelfRatchetColumns<
...
impl<const MAX_RATCHETED_DESTINATIONS: usize, const RETAINED_RATCHETS_PER_DESTINATION: usize>
    Default
    for FixedSelfRatchetColumns<MAX_RATCHETED_DESTINATIONS, RETAINED_RATCHETS_PER_DESTINATION>
{
    fn default() -> Self {
        Self {
...
mod tests {
    ...
    fn exposes_only_pushed_rows_and_reports_a_full_table() {
        let mut columns = FixedSelfRatchetColumns::<2, 3>::default();
        ...
        assert_eq!(columns.secrets_newest_first(0).map(<[_]>::len), Some(0));
        assert!(columns.secrets_newest_first(2).is_none());
    }

    #[test]
    fn inserts_keep_newest_first_and_trim_the_oldest_at_the_retained_cap() {
        let mut columns = FixedSelfRatchetColumns::<1, 2>::default();
        columns.push(dest(1)).unwrap();

        columns.insert_newest_secret(0, secret(0x11));
        columns.insert_newest_secret(0, secret(0x22));
        assert_eq!(
            secret_bytes(columns.secrets_newest_first(0).unwrap()),
            &[public(0x22), public(0x11)],
        );

        columns.insert_newest_secret(0, secret(0x33));
        assert_eq!(
            secret_bytes(columns.secrets_newest_first(0).unwrap()),
    ...
    fn out_of_range_writes_are_ignored() {
        ...
        columns.insert_newest_secret(5, secret(0x11));
        columns.set_last_rotated(5, InstantMillis(1_000));

        assert_eq!(columns.secrets_newest_first(0).map(<[_]>::len), Some(0));
        assert_eq!(columns.last_rotated(), &[LastRotated::Never]);
    }
    ...
    fn set_last_rotated_anchors_the_row() {
        let mut columns = FixedSelfRatchetColumns::<1, 2>::default();
        columns.push(dest(1)).unwrap();
        columns.set_last_rotated(0, InstantMillis(7_000));
        assert_eq!(
            columns.last_rotated(),
