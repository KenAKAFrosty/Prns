...
recovery instructions. Fallback and installer assignments separately confirm access to their
exact browser or target archive. Do not claim readiness that has not been confirmed.
...
Final acceptance requires each physical, browser-fallback, and native-installer row to match its
exact assignment. An unlisted identity or substituted host cannot satisfy the matrix even when
every scenario is marked passing.
