...
test("holds later event batches until the page acknowledges the in-flight batch", async () => {
  ...
  assert.equal(first.type, "batch");
  const secondMessage = nextMessage(channel.port2);
  channel.port2.postMessage({ type: "acknowledge", id: first.id });
  const second = await secondMessage;
  assert.equal(second.type, "batch");
  channel.port2.postMessage({ type: "acknowledge", id: second.id });
...
test("coalesces diagnostics dropped behind a full event channel", async () => {
  ...
  const first = await firstMessage;
  assert.deepEqual(first.event, Tag("Delivered", { detail: "first" }));
  const gapMessage = nextMessage(channel.port2);
  channel.port2.postMessage({ type: "acknowledge", id: first.id });
  const gap = await gapMessage;
  assert.deepEqual(gap.event, Tag("DiagnosticsDropped", { count: 1n }));
  channel.port2.postMessage({ type: "acknowledge", id: gap.id });
  assert.deepEqual(failures, []);
  channel.port1.close();
  channel.port2.close();
});
