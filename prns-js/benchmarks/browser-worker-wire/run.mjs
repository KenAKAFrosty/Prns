...
assert.ok(chromium, "Chromium is required for the browser worker wire benchmark");
...
const server = createServer(async (request, response) => {
  try {
    ...
    if (request.method === "POST" && url.pathname === "/browser-worker-wire-result") {
      const chunks = [];
...
try {
  ...
  const url = `http://127.0.0.1:${address.port}/benchmarks/browser-worker-wire/index.html`;
  ...
  const timeout = new Promise((_, reject) => {
    benchmarkTimeout = setTimeout(
      () => reject(new Error("browser worker wire benchmark timed out")),
      120_000,
    );
  });
