...
( cd "$ROOT/personal-rns-interfaces/tokio" && cargo build --quiet --example local_transit_daemon --features "shared-instance tcp" ) \
...
PRNS_LOCAL_PORT="$LOCAL_PORT" PRNS_PEER_ADDR="127.0.0.1:$PEER_TCP_PORT" \
    "$ROOT/personal-rns-interfaces/tokio/target/debug/examples/local_transit_daemon" > "$DAEMON_LOG" 2>&1 &
DAEMON_PID=$!
