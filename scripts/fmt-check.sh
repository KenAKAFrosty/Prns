...
#
# fmt is checked across the root workspace and each host workspace separately —
# the hosts are their own workspaces (own targets/toolchains), so `--all` from
# the root doesn't reach them. The doc step builds personal-rns's docs with
...
for host in hosts/esp32-c6 hosts/heltec-lora32 hosts/nrf52840; do
  echo "[fmt] ${host}"
