...
def main() -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="prns-historical-verifier-") as temporary:
            ...
            verifier = existing(
                snapshot / "scripts/verify-flasher-release.sh", "historical release verifier"
            )
            asset_verifier = existing(
                snapshot / "scripts/verify-flasher-release-assets.py",
                "historical release-asset verifier",
            )
            public_review_verifier = existing(
                snapshot / "scripts/flasher-public-review.py",
