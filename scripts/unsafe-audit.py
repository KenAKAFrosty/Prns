...
def build_snapshot() -> dict:
    ...
    return {
        "schema": 1,
        "generator": "scripts/unsafe-audit.py",
        "graphs": [
...
'''
    ...
    if not BASELINE.exists():
        print("unsafe baseline missing; run scripts/unsafe-audit.py --write", file=sys.stderr)
        return 1
    expected = BASELINE.read_text(encoding="utf-8")
    if expected != rendered:
        print("unsafe dependency snapshot drifted; review and run scripts/unsafe-audit.py --write", file=sys.stderr)
        return 1
