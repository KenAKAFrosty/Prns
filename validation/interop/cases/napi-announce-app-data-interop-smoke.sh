...
PRNS_ANNOUNCE_APP_DATA_PORT="$PORT" \
PRNS_ANNOUNCE_APP_DATA_CONFIG_DIR="$WORK/stock-rns" \
    "$PYTHON" "$STOCK_PEER" >"$STOCK_LOG" 2>&1 &
STOCK_PID=$!
for _ in $(seq 1 100); do
    grep -q "ANNOUNCE_APP_DATA_PEER_UP" "$STOCK_LOG" && break
    kill -0 "$STOCK_PID" 2>/dev/null || break
    sleep 0.1
done
grep -q "ANNOUNCE_APP_DATA_PEER_UP" "$STOCK_LOG" || { echo "FAIL: stock announce application data peer did not start"; cat "$STOCK_LOG"; exit 1; }
...
if grep -qF "$STOCK_OK" "$STOCK_LOG" && grep -qF "$NAPI_OK" "$NAPI_LOG"; then
    echo "PASS: stock RNS 1.4.2 and Prns each preserved exact opaque announce application bytes"
    exit 0
fi

echo "FAIL: bidirectional announce application data evidence did not complete"
cat "$STOCK_LOG"
cat "$NAPI_LOG"
exit 1
