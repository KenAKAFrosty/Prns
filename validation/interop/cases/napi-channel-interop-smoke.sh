...
SERVER="$ROOT/validation/interop/peers/rns_channel_server.py"
CLIENT="$ROOT/prns-napi/tests/interop/channel_client.mjs"
SERVER_LOG="$(mktemp)"
CLIENT_LOG="$(mktemp)"
SERVER_PID=""
CLIENT_PID=""
SERVER_OK="STOCK_CHANNEL_OK messages=2 ordered=1"
CLIENT_OK="NAPI_CHANNEL_OK messages=2 ordered=1 proven=2"
...
PRNS_CHANNEL_PORT="$PORT" "$PYTHON" "$SERVER" >"$SERVER_LOG" 2>&1 &
SERVER_PID=$!
for _ in $(seq 1 100); do
    grep -q "CHANNEL_SERVER_UP" "$SERVER_LOG" && break
    kill -0 "$SERVER_PID" 2>/dev/null || break
    sleep 0.1
done
grep -q "CHANNEL_SERVER_UP" "$SERVER_LOG" || { echo "FAIL: stock RNS Channel server did not start"; cat "$SERVER_LOG"; exit 1; }
...
if grep -qF "$SERVER_OK" "$SERVER_LOG" && grep -qF "$CLIENT_OK" "$CLIENT_LOG"; then
    echo "PASS: stock RNS 1.4.2 and Prns exchanged ordered, proven Channel messages in both directions"
    exit 0
fi

echo "FAIL: bidirectional stock-RNS Channel exchange did not complete"
cat "$SERVER_LOG"
cat "$CLIENT_LOG"
exit 1
