...
| Legacy stock-RNS environments | Both `benchmarks/reference/.venv` and `.rpc-venv` contained RNS 1.3.9, not the required 1.4.0. | Ordinary tests no longer discover them. `prepare-oracles` creates ignored, version-checked RNS 1.4.0 environments under `validation/.venv/`; legacy environments are cleanup candidates. |
