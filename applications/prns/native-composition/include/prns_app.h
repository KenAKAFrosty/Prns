...
/**
 * Synchronously create the application-owned CoreBluetooth central restoration
 * manager and delegate without waiting for full node readiness. A matching
 * restoring start atomically consumes the retained prepared owner.
 */
PrnsAppBytes prns_app_prepare_apple_bluetooth_central_restoration(
    const uint8_t *path_ptr, size_t path_len,
    const uint8_t *central_identifier_ptr, size_t central_identifier_len);

/**
 * Start the development node with a stable, application-owned CoreBluetooth
 * central restoration identifier. All buffers are length-delimited UTF-8 and
 * the central identifier must be nonempty.
 */
PrnsAppBytes prns_app_start_with_apple_bluetooth_central_restoration(
    const uint8_t *path_ptr, size_t path_len, const uint8_t *input_ptr,
    size_t input_len, const uint8_t *central_identifier_ptr,
    size_t central_identifier_len);
