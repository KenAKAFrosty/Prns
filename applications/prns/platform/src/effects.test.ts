...
function fakeRuntime(overrides: Partial<DevelopmentRuntime> = {}): DevelopmentRuntime {
  return {
    ...
    startRemoteWifiTrial: jest.fn(async () => Bindings.RemoteWifiCommandOutcome.Busy.new()),
    inspectRemoteWifiTrial: jest.fn(async () => Bindings.RemoteWifiCommandOutcome.Busy.new()),
    finishRemoteWifiTrial: jest.fn(async () => Bindings.RemoteWifiCommandOutcome.Busy.new()),
...
describe("Effect development runtime orchestration", () => {
  test("scopes one runtime, ignores stale revisions, and cancels sequential refresh", async () => {
    ...
    const observedSecond = new Promise<void>((resolve) => {
        ...
        Effect.gen(function* () {
          yield* scopedDevelopmentRuntime(runtime, {
            ...
            onSnapshot: (current) => {
              ...
              if (current.revision === 2n) resolveSecond?.();
            },
            onBackgroundFailure: (failure) => failures.push(failure),
          });
          yield* Effect.promise(() => observedSecond).pipe(Effect.timeout(2000));
        }),
      ),
    );
