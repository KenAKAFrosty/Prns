...
import type * as NativeBindings from "@prns-internal/native-bindings/native";
...
jest.mock("../../native-composition/bindings/typescript/prns_app-ffi", () => ({
  __esModule: true,
  default: () => ({
...
function setup(overrides: Partial<typeof NativeBindings> = {}, native = nativeModule()) {
  const api = {
    ...Bindings,
    ...
    borrowHost: jest.fn(() => {
      throw new Error("Unexpected host attachment");
    }),
    ...overrides,
  };
...
test("attaches a shared host through the loaded native SDK entry", async () => {
  const handle = {} as NonNullable<ReturnType<typeof Bindings.sharedHost>>;
  const client = {} as ReturnType<typeof NativeBindings.borrowHost>;
  const sharedHost = jest.fn(() => handle);
  const borrowHost = jest.fn(() => client);
  const { runtime, load } = setup({ sharedHost, borrowHost });

  await expect(runtime.attachHost()).resolves.toBe(client);
  expect(sharedHost).toHaveBeenCalledTimes(1);
  expect(borrowHost).toHaveBeenCalledWith(handle);
  expect(load).toHaveBeenCalledTimes(1);
});

test("does not wrap a host when the native owner is absent", async () => {
  const { runtime, api } = setup({ sharedHost: jest.fn(() => undefined) });

  await expect(runtime.attachHost()).resolves.toBeUndefined();
  expect(api.borrowHost).not.toHaveBeenCalled();
});
