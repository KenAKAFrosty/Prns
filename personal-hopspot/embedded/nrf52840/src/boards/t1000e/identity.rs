//! T1000-E identity-vault layout and bootstrap policy.
//!
//! The T1000-E shares the T-Echo's nRF52840 + S140 SoftDevice + memory.x, so the
//! on-chip flash partitioning is byte-identical (see `personal_hopspot_core::flash_layout`:
//! `T1000E_*` alias `T_ECHO_*`). The reserved offsets below match the T-Echo until a
//! distinct T1000-E layout is laid down.
