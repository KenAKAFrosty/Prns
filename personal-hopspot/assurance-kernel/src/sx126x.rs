...
pub async fn run(transcript: &RefCell<Transcript>) -> Result<(), ScenarioError> {
    ...
    radio
        ...
        .await
        .map_err(ScenarioError::Sx126x)?;
    trace
        .record(EventKind::Result, &[1])
        .map_err(|_| ScenarioError::TranscriptFull)?;
    radio
        .transmit(b"sx126x-target")
        .await
        .map_err(ScenarioError::Sx126x)?;
    trace
    ...
    let timeout = match radio.transmit(b"sx126x-timeout").await {
    ...
    radio.arm_rx().await.map_err(ScenarioError::Sx126x)?;
