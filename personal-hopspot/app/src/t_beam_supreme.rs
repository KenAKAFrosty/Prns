...
pub async fn run(spawner: Spawner) {
    ...
    println!("HOPSPOT_TBEAM_SUPREME boot — recipe runtime, engine core 1 + I/O core 0");

    // AXP2101 PMU first (I2C1 on SDA 42 / SCL 41): the LoRa + OLED rails boot OFF, so nothing else
    // on those rails responds until this enables them. The handle is dropped after — the rails latch.
    ...
    let mut battery_source = Axp2101Battery::new(pmu_i2c);
    {
        use screen::BatterySource as _;
        println!("battery: {:?} mV", battery_source.read_millivolts());
    }
    ...
    // Battery on the T-Beam S3 Supreme is read over the AXP2101's fuel-gauge ADC, not a GPIO divider
    // (and GPIO1 here is the LoRa DIO1), so battery telemetry is deferred: the card reads Unknown
    // until the PMU ADC read is wired up.
